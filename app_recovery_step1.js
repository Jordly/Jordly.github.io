...
var DEFAULT_ISSUES = [
  ...
  {id:4, projectId:"P001", projectName:"缇庡鏃楄埌搴楀鏈嶉」鐩?, category:"闂", type:"浼樺寲", desc:"澶т績棰勬闇€瑕佹洿鏂帮紝鍘诲勾鍙?1鍑虹幇浜烘墜涓嶈冻", priority:"涓€鑸?, owner:"寮犱紵", assignee:"寮犱紵", status:"宸插叧闂?, source:"浜哄伐涓婃姤", responsibility:"鎵挎帴鏂?, createdAt:"2026-04-01", solution:"宸插畬鎴愬ぇ淇冧汉鍔涢妗堬紝澧炲姞20%涓存椂浜哄姏鍌ㄥ", background:"2025骞村弻11鏈熼棿宄板€煎湪绾块噺瓒呭嚭鎺掔彮120%锛屽钩鍧囧搷搴旀椂闂撮鍗囪嚦210s", rootCause:"鍘嗗彶鏁版嵁鍙傝€冩ā鍨嬫湭鑰冭檻鐩存挱甯﹁揣鏂版笭閬撳紩娴?, milestone:"", outcome:"", participants:"鍩硅閮?},
  ...
  {id:8, projectId:"P005", projectName:"鏈嶈鍝佺墝瀹㈡湇澶栧寘", category:"璇鹃", type:"鏈嶅姟鍗囩骇", desc:"VIP瀹㈡埛涓撳睘鏈嶅姟閫氶亾寤鸿 鈥斺€?涓撳睘瀹㈡湇+48h鍥炶+瀛ｅ害鍏虫€€璁″垝", priority:"all", owner:"闄堥潤", assignee:"璧电", status:"鎵ц涓?, source:"鎴樼暐瑙勫垝", responsibility:"鎵挎帴鏂?, createdAt:"2026-04-20", solution:"", background:"TOP200瀹㈡埛璐＄尞45%閿€鍞浣嗘棤涓撳睘鏈嶅姟鏉冪泭锛屽璐巼鍚屾瘮涓嬮檷5%", rootCause:"", milestone:"瀹㈡埛鍒嗙骇鈫掓湇鍔℃潈鐩婅璁♀啋绯荤粺閰嶇疆鈫掑皬鑼冨洿璇曠偣(褰撳墠)", outcome:"鐩爣澶嶈喘鐜囧洖鍗囪嚦鍘诲勾鍚屾湡姘村钩", participants:"CRM绯荤粺鍥㈤槦"},
...
var DEFAULT_USERS = [
  {id:"U001", name:"绯荤粺鍒涘缓鑰?, username:"admin", role:"瓒呯骇绠＄悊鍛?, status:"宸叉縺娲?, registerTime:"2025-01-01", password:"admin123", phone:"18510084943", email:"zhoudongli@xcxd.com", nickname:"Jordly", birthday:"1991-12-18", position:"瀹㈡湇", brand:"娴庡崡/娣勫崥/鏉窞", approvedBy:"system", remark:"绯荤粺鍒濆鍖栬秴绾х鐞嗗憳"},
    ...
    <div style="display:flex;gap:0;">
      <div class="tl-container" style="flex:1;max-height:280px;overflow-y:auto;padding-right:8px;">
        ${(function(){
          ...
          for (var i=0;i<rp.length;i++) { events.push({type:'new',icon:'馃啎',label:'鏂板椤圭洰',clr:'#22c55e',bg:'rgba(34,197,94,0.08)',title:rp[i].name,detail:(rp[i].serviceMode||'')+' 路 '+(rp[i].workplace||'')+' 路 PM锛?+(rp[i].pm||''),time:'鏈€杩?}); }
          var urg = ISSUES.filter(function(x){return x.priority==='绱ф€?;}).slice(0,1);
          for (var i=0;i<urg.length;i++) { events.push({type:'warning',icon:'鈿狅笍',label:'鎶曡瘔/棰勮',clr:'#f59e0b',bg:'rgba(245,158,11,0.08)',title:urg[i].projectName+'锛?+urg[i].desc,detail:'浼樺厛绾э細绱ф€?路 鐘舵€侊細'+urg[i].status+' 路 璐ｄ换浜猴細'+urg[i].assignee,time:'鏈€杩?}); }
          var hv = HANDOVERS.filter(function(x){return x.status==='宸插畬鎴?;}).sort(function(a,b){return (b.date||'').localeCompare(a.date||'');}).slice(0,1);
          for (var i=0;i<hv.length;i++) { events.push({type:'transfer',icon:'馃攧',label:'PM浜ゆ帴',clr:'#3b82f6',bg:'rgba(59,130,246,0.08)',title:hv[i].projectName+'锛?+hv[i].from+' 鈫?'+hv[i].to,detail:'浜ゆ帴鏃ユ湡锛?+hv[i].date+' 路 鐘舵€侊細宸插畬鎴?,time:hv[i].date||'鏈€杩?}); }
          var cl = ISSUES.filter(function(x){return x.status==='宸插叧闂?;}).slice(0,1);
          for (var i=0;i<cl.length;i++) { events.push({type:'done',icon:'鉁?,label:'闂鍏抽棴',clr:'#9ca3af',bg:'rgba(156,163,175,0.08)',title:cl[i].projectName+'锛?+cl[i].desc,detail:'鍏抽棴浜猴細'+(cl[i].assignee||'绯荤粺')+' 路 鏉ユ簮锛?+(cl[i].source||''),time:'鏈€杩?}); }
    ...
    +'<div class="modal-dialog" style="max-width:700px;padding:0;border-radius:16px;overflow:hidden;background:#fff;">'
      ...
      +'<div style="padding:20px;">'
        ...
        +'<div style="margin-top:12px;text-align:center;font-size:11px;color:#94a3b8;">'
          +(s1>s2 ? p1.name+' 缁煎悎棰嗗厛 '+ (s1-s2)+' 鍒? : (s2>s1 ? p2.name+' 缁煎悎棰嗗厛 '+ (s2-s1)+' 鍒? : '鍙屾柟缁煎悎寰楀垎鎸佸钩'))
        +'</div>'
      +'</div>'
    +'</div>';
  overlay.classList.remove('hidden');
...
function renderIssue(){
  ...
  if(isTopic){
    ...
    tableHtml = '<table class="issue-table"><thead><tr>'
      +'<th style="width:8%;">鐘舵€?/th><th style="width:9%;">缂栧彿</th><th style="width:11%;">椤圭洰</th><th style="width:9%;">璇鹃绫诲瀷</th>'
      +'<th style="width:28%;">鐩爣鎻忚堪</th><th style="width:14%;">鍏抽敭鑺傜偣</th><th style="width:7%;">鍗忓悓鏂?/th><th style="width:14%;text-align:center;">鎿嶄綔</th></tr></thead><tbody>'
    ...
    tableHtml = '<table class="issue-table"><thead><tr>'
      +'<th style="width:8%;">鐘舵€?/th><th style="width:9%;">缂栧彿</th><th style="width:11%;">椤圭洰</th><th style="width:8%;">绫诲瀷</th>'
      +'<th style="width:25%;">闂鎻忚堪</th><th style="width:7%;text-align:center;">浼樺厛绾?/th><th style="width:7%;">璐ｄ换浜?/th><th style="width:7%;">褰掑睘</th><th style="width:18%;text-align:center;">鎿嶄綔</th></tr></thead><tbody>'
      +filtered.map(function(i){
  ...
  if(isTopic){
    filterHtml = ''
      +'<select class="fb-select" onchange="filterIssues()" id="issue-filter-type" style="flex:1;min-width:120px;max-width:180px;">'
        ...
        +'<option value="娴佺▼浼樺寲">娴佺▼浼樺寲</option><option value="璋冪爺璇婃柇">璋冪爺璇婃柇</option><option value="閿€鍞彁鍗?>閿€鍞彁鍗?/option><option value="鏈嶅姟鍗囩骇">鏈嶅姟鍗囩骇</option><option value="鎴愭湰浼樺寲">鎴愭湰浼樺寲</option><option value="椋庨櫓闃叉帶">椋庨櫓闃叉帶</option></select>'
      ...
      +'</select>'
      +'<input type="text" id="issue-search" placeholder="鎼滅储璇鹃鎻忚堪..." oninput="filterIssues()" style="flex:2;min-width:140px;max-width:240px;padding:7px 12px;border-radius:10px;border:1px solid #d0d7de;font-size:13px;background:linear-gradient(135deg,#f5f3ff,#ede9fe,#dbeafe);color:#334155;">';
  } else {
    filterHtml = ''
      ...
      +'<select class="fb-select" onchange="filterIssues()" id="issue-filter-type" style="flex:1;min-width:100px;max-width:160px;">'
        +'<option value="all">绫诲瀷 鈻?/option><option value="鏁存敼">鏁存敼</option><option value="瀹㈣瘔">瀹㈣瘔</option><option value="鏁版嵁寮傚父">鏁版嵁寮傚父</option><option value="娴佺▼鍗＄偣">娴佺▼鍗＄偣</option><option value="绯荤粺鏁呴殰">绯荤粺鏁呴殰</option><option value="浼樺寲">浼樺寲</option><option value="鍏朵粬">鍏朵粬</option></select>'
      ...
      +'</select>'
      +'<input type="text" id="issue-search" placeholder="鎼滅储闂鎻忚堪..." oninput="filterIssues()" style="flex:2;min-width:140px;max-width:240px;padding:7px 12px;border-radius:10px;border:1px solid #d0d7de;font-size:13px;background:linear-gradient(135deg,#f0fdf4,#ecfdf5,#dbeafe);color:#334155;">';
  }
  ...
  // 鍙屾爣绛惧垏鎹?  +'<div style="display:flex;gap:12px;margin-bottom:16px;">'
    +'<div class="issue-tab-pill '+(issueActiveTab==='issue'?'active':'')+'" onclick="switchIssueTab(\'issue\')" style="flex:1;cursor:pointer;padding:14px 18px;border-radius:14px;text-align:center;transition:all 0.25s;'
      ...
      +'<div style="font-size:11px;margin-top:4px;opacity:0.8;">'+(issueActiveTab==='issue'?'褰撳墠娲昏穬 路 ':'')+issueTotal+'鏉¤褰?/div>'
    +'</div>'
    +'<div class="issue-tab-pill '+(issueActiveTab==='topic'?'active':'')+'" onclick="switchIssueTab(\'topic\')" style="flex:1;cursor:pointer;padding:14px 18px;border-radius:14px;text-align:center;transition:all 0.25s;'
      ...
      +'<div style="font-size:11px;margin-top:4px;opacity:0.8;">'+(issueActiveTab==='topic'?'褰撳墠娲昏穬':'鐐瑰嚮鍒囨崲')+' 路 '+topicTotal+'鏉¤褰?/div>'
    +'</div>'
  +'</div>'
    ...
    +'<div class="modal-dialog" style="max-width:680px;padding:0;border-radius:16px;overflow:hidden;background:#fff;">'
      +'<div style="background:'+headerGrad+';color:'+headerTextColor+';padding:14px 20px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid '+headerBorder+';">'
        +'<div>'
          ...
          +'<div style="font-size:11px;opacity:0.7;">'+(isTopic?'T':'I')+String(item.id).padStart(3,'0')+' 路 '+item.projectName+'</div>'
        +'</div>'
      ...
      +'<div style="padding:20px;">'
        +'<div style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap;">'
          ...
          +'<div style="flex:1;min-width:120px;padding:10px;background:#f8fafc;border-radius:10px;">'
            +'<div style="font-size:10px;color:#94a3b8;">绫诲瀷</div><div style="font-weight:600;">'+item.type+'</div></div>'
          +'<div style="flex:1;min-width:120px;padding:10px;background:#f8fafc;border-radius:10px;">'
        ...
        +(item.background?'<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:600;color:#1e40af;margin-bottom:4px;">鑳屾櫙</div><div style="font-size:12px;color:#475569;line-height:1.6;">'+item.background+'</div></div>':'')
        +(item.rootCause?'<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:600;color:#ef4444;margin-bottom:4px;">鏍瑰洜鍒嗘瀽</div><div style="font-size:12px;color:#475569;line-height:1.6;">'+item.rootCause+'</div></div>':'')
        ...
        +(item.milestone?'<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:600;color:#0B9B96;margin-bottom:4px;">鍏抽敭鑺傜偣</div><div style="font-size:12px;color:#475569;line-height:1.6;">'+item.milestone+'</div></div>':'')
        ...
        +'<div style="color:#94a3b8;font-size:10px;">鏉ユ簮锛?+(item.source||'')+' 路 鍒涘缓锛?+(item.createdAt||'')+'</div>'
      +'</div>'
    +'</div>';
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(ev) {
      try {
        ...
        showToast('鉁?瀵煎叆鎴愬姛锛佸鏈嶉厤缃?' + imported.staff + '鏉★紝宸ヤ綔閲?' + imported.workload + '鏉★紝KPI鍘嗗彶+' + imported.kpi + '鏉?);
        renderDashboard();
      } catch(err) {
...
function runComparison(){
  ...
  // 闆疯揪鍥撅紙绠€鏄撴枃瀛楃増锛?  html += `<div class="card"><div class="card-title">馃搱 鍏抽敭鎸囨爣瀵规瘮</div><div style="display:flex;gap:16px;flex-wrap:wrap;">`;
  projects.forEach(p=>{
    const op = OPERATIONS.find(o=>o.projectId===p.id);
...
function switchDataTab(tabIdx) {
  ...
  var html = '';
  if (tabIdx === 0) {
    html += '<table style="width:100%;border-collapse:collapse;"><tr style="background:var(--c-bg-2);"><th style="padding:6px;border:1px solid var(--c-border);">瑙掕壊</th><th style="padding:6px;border:1px solid var(--c-border);">浜烘暟</th><th style="padding:6px;border:1px solid var(--c-border);">鍗犳瘮(%)</th><th style="padding:6px;border:1px solid var(--c-border);">鎿嶄綔</th></tr>';
    ...
    html += '<table style="width:100%;border-collapse:collapse;"><tr style="background:var(--c-bg-2);"><th style="padding:6px;border:1px solid var(--c-border);">宸ヤ綔绫诲瀷</th><th style="padding:6px;border:1px solid var(--c-border);">鏁伴噺</th><th style="padding:6px;border:1px solid var(--c-border);">鍗犳瘮(%)</th><th style="padding:6px;border:1px solid var(--c-border);">鎿嶄綔</th></tr>';
    ...
    html += '<table style="width:100%;border-collapse:collapse;"><tr style="background:var(--c-bg-2);"><th style="padding:6px;border:1px solid var(--c-border);">鏈堜唤</th><th style="padding:6px;border:1px solid var(--c-border);">閿€鍞</th><th style="padding:6px;border:1px solid var(--c-border);">鎴愭湰</th><th style="padding:6px;border:1px solid var(--c-border);">璐规晥姣?/th><th style="padding:6px;border:1px solid var(--c-border);">鐩爣杈炬垚鐜?/th><th style="padding:6px;border:1px solid var(--c-border);">鎿嶄綔</th></tr>';
...
// 鏁版嵁琛ㄥ畾涔夛紙鍖呭惈瀛楁瀹氫箟锛岀敤浜庢柊澧?缂栬緫琛ㄥ崟锛?var SYSTEM_DATA_TABLES = {
  projects: {
    ...
    fields: [
      ...
      {key:'serviceMode', label:'椤圭洰绫诲瀷', type:'select', options:['TP椤圭洰','DP椤圭洰','鑷惀椤圭洰']},
  ...
  operations: {
    ...
    data: OPERATIONS,
    fields: [
      {key:'projectId', label:'椤圭洰ID', type:'text', required:true},
      ...
      {key:'responseTime', label:'鍝嶅簲鏃堕棿(s)', type:'number'},
      {key:'handleDuration', label:'澶勭悊鏃堕暱(h)', type:'number'},
  ...
  handovers: {
    label: '鈴?浜ゆ帴璁板綍琛?,
    desc: '椤圭洰PM浜ゆ帴鐨勫巻鍙茶褰曪紝鍖呭惈浜ゆ帴浜恒€佹棩鏈熴€佸畬鎴愮姸鎬佺瓑銆傞」鐩壙鎺ヨ鑼冮〉闈緷璧栨琛ㄣ€?,
    data: HANDOVERS,
    fields: [
      {key:'id', label:'ID', type:'text', required:true},
      {key:'projectId', label:'椤圭洰ID', type:'text'},
  ...
  changelog: {
    ...
    fields: [
      ...
      {key:'recordId', label:'璁板綍ID', type:'text'},
...
function renderSystemData(){
  ...
  if(_systemDataView === 'catalog'){
    ...
    <div class="module-header">
      <div>
        ...
        <div style="font-size:12px;color:var(--c-text-3);margin-top:4px;">缁熶竴鏁版嵁绠＄悊涓績 路 鎵€鏈夋暟鎹殑鍞竴鍏ュ彛涓庣淮鎶や腑蹇?/div>
      </div>
    </div>
  ...
  if(_systemDataTab === 'projects'){
    ...
    headers = ['<input type="checkbox" onchange="toggleSelectAll(this)">','椤圭洰ID','宸ュ崟閲?,'杞寲鏁?,'瀹㈠崟浠?,'婊℃剰搴?,'鍝嶅簲鏃堕棿','澶勭悊鏃堕暱','鐘舵€?];
    ...
    headers = ['<input type="checkbox" onchange="toggleSelectAll(this)">','ID','椤圭洰ID','椤圭洰鍚嶇О','鍘熻礋璐ｄ汉','鏂拌礋璐ｄ汉','鏃ユ湡','鐘舵€?,'鎽樿'];
  } else if(_systemDataTab === 'kpi'){
    headers = ['<input type="checkbox" onchange="toggleSelectAll(this)">','鏃ユ湡','椤圭洰ID','閿€鍞(涓?','鎴愭湰(涓?','璐规晥姣?,'鐩爣杈炬垚鐜?];
  } else if(_systemDataTab === 'changelog'){
    headers = ['鏃堕棿','鎿嶄綔浜?,'琛ㄥ悕','璁板綍ID','瀛楁鍚?,'鏃у€?,'鏂板€?];
  ...
  <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;align-items:center;">
    ...
    ${!isLog?'<button class="btn btn-sm" onclick="searchSystemData()">鎼滅储</button><button class="btn btn-sm" onclick="_systemDataSearchKeyword=\'\';_systemDataPage=1;var s=document.getElementById(\'system-data-search\');if(s)s.value=\'\';renderModule(\'systemData\')">娓呴櫎</button>':''}
