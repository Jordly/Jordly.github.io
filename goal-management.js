...
function renderTarget() {
  ...
  <div class="module-header">
    ...
    <div class="module-actions">
      ...
      <button class="btn btn-sm" onclick="exportGoalsCSV()" style="margin-left:4px;">📥 导出</button>
...
function exportGoalsCSV() {
  ...
  GOALS.forEach(function(g) {
    ...
    var row = [
      g.id,
      esc(projectNameMap[g.projectId] || g.projectId),
      g.type,
      esc(g.target),
      esc(g.metric || ''),
      esc(g.owner || ''),
  ...
  var ds = new Date();
  var dsStr = ds.getFullYear() + '' + String(ds.getMonth() + 1).padStart(2, '0') + '' + String(ds.getDate()).padStart(2, '0');
  a.href = URL.createObjectURL(blob);
  a.download = '目标与权责_' + dsStr + '.csv';
  a.click();
