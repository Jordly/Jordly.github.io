    ...
    reader.onload = function(e){
      try {
        ...
        for(var i=1;i<rows.length;i++){
          ...
          var health = (vals[idx['健康度']]||'').toString().trim();
          ...
          var obj = {
            ...
            name: name,
            brand: (vals[idx['品牌']]||'').toString().trim()||'未知',
            category: (vals[idx['品类']]||'').toString().trim()||'未分类',
            serviceMode: (vals[idx['项目类型']]||'').toString().trim()||'TP项目',
            workplace: (vals[idx['所属职场']]||'').toString().trim()||'济南',
            pm: (vals[idx['项目经理']]||'').toString().trim()||'未分配',
            director: (vals[idx['项目总监']]||'').toString().trim()||'未分配',
            status: status||'优质健康店',
            health: health||'🟢',
            startDate: (vals[idx['服务周期开始']]||'').toString().trim()||new Date().toISOString().slice(0,10),
            endDate: (vals[idx['服务周期结束']]||'').toString().trim()||'2026-12-31',
            base: (vals[idx['客服base']]||'').toString().trim()||'',
            platforms: (vals[idx['服务渠道']]||'').toString().trim()||'',
            serviceHours: (vals[idx['服务时间']]||'').toString().trim()||'09:00-22:00',
            ...
            background: (vals[idx['项目背景']]||'').toString().trim()||'',
            pmHistory: []
          };
          if(existing){
        ...
        console.error('[import] Excel解析失败:', err);
