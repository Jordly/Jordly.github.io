...
function renderCost(){
  ...
  const avgProfit = all.length ? (all.reduce((s,p)=>s+((p.revenue&&p.revenue>0)?((p.revenue-Math.round((p.costBudget||0)*(0.9+Math.random()*0.3))/p.revenue*100):0),0)/all.length) : 0;
