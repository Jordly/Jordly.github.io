...
//
// ══════════════════════════════════════════════════════════════
// 📍 代码地图（查找指南 · 2026-08-14 更新 · app.js 主框架）
// ══════════════════════════════════════════════════════════════
// 说明：不再标注行号（易过时），改为「区块 → 职责」；定位函数请用编辑器全局搜索函数名。
// ── app.js 主框架职责分区 ──
//   数据层：Mock演示数据 / 薪资配置 / 数据持久化(safeSetItem等) / 看板数据模型
//   账户：登录注册 / 密码加密(PBKDF2)
//   权限：角色与权限 / 导航过滤(统一依赖 app.js 全局 getPermission + canViewModule 严格判定)
//   框架：UI框架(侧边栏/导航) / 模块分发(renderModule) / 筛选栏 v4 / 项目多选搜索 / 驾驶舱卡片弹窗
// ── 安全基础设施（独立文件）──
//   core-security.js：XSS转义(escHtml) + PBKDF2密码哈希 + 全局错误捕获 + 环形运行日志 + 一键备份(backupAllData)
// ── 业务模块（modules/ 目录，各文件独立函数，注意全局命名空间不重名）──
//   dashboard/archive/target/cost/operation/issue/knowledge/handover/
//   satisfaction/notifications/systemData/permissions/assessment/performance/
//   risk/profile/roster
// ── 其他独立文件 ──
//   backup-restore.js / cloudbase-sync.js / project-import-export.js /
//   premium-enhancements.js / goal-management.js / filterBarNew.js
// ══════════════════════════════════════════════════════════════
//
// ===== Mock 数据 =====
